...
class ControlProcess:
	...
	def executeFunc(self,data,rule):
		...
		rule_columns = ['control_id','control_name','prog_id','name','person_type','target_id','idcard','last_device_id','last_col_time','police','control_prog_id','control_prog','prog_target','prog_rule','target_type','target_json','start_time','end_time','leader_state','emergency_grade','create_user','create_time','update_user','update_time','is_delete']
		gather_r_df = rule[rule['control_prog_id'].isin(['5'])]
		pos_r_df = rule[rule['control_prog_id'].isin(['10','11'])]
		per_r = rule[rule['control_prog_id'].isin(['1','2','3','4','6','7','8'])]
