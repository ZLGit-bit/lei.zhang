...
import datetime
import MysqlUtils
from Process import ControlProcess
...
def process(data):
	...
	if not ori_data.isEmpty():
	    ...
	    idcards = '"%s"' % '","'.join(set(ori_df['idcard'].values.tolist()))
	    #print idcards
	    rule_sql = mysql_conf['control_sql'] % idcards
	    rule_df = MysqlUtils.get_data( mysql_conf,rule_sql)
	    if ori_df.shape[0]>0:
	        print 'start->process'
